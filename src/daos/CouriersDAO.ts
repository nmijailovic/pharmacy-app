...
const log = require('bows')('API:DAO', 'Couriers');

export class CouriersDAO extends BaseDAO {
  constructor() {
    // no op
    super();
  }
  ...
  public async list(token: IToken, parentWsIds: IKVObject, params: ICouriersParams): Promise<IApiPagedResponse<ICouriersWS>> {
    try {
  ...
  public async get(token: IToken, parentWsIds: IKVObject, params: ICouriersParams): Promise<ICouriersWS> {
    try {
