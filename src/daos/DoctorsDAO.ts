...
const log = require('bows')('API:DAO', 'Doctors');

export class DoctorsDAO extends BaseDAO {
  constructor() {
    // no op
    super();
  }
  ...
  public async list(token: IToken, parentWsIds: IKVObject, params: IDoctorsParams): Promise<IApiPagedResponse<IDoctorsWS>> {
    try {
      ...
      const results = await models.Doctors.findAndCountAll({
  ...
  public async get(token: IToken, parentWsIds: IKVObject, params: IDoctorsParams): Promise<IDoctorsWS> {
    try {
      ...
      const result = await models.Doctors.findOne({
        where: searchObj.where,
        // GQL requires Sub-Object IDs
        include: this.getJoinIncludeAry(searchObj)
      });
  ...
  public async create(token: IToken, parentWsIds: IKVObject, ws: IDoctorsWS): Promise<IDoctorsWS> {
    try {
      ...
      const result = await models.Doctors.create({
        // ws fields
        ...ws,
        // parent resource IDs
        ...parentWsIds,
  ...
  public async update(token: IToken, parentWsIds: IKVObject, id: number, ws: IDoctorsWS): Promise<number> {
    try {
      ...
      const result = await models.Doctors.update(
        {
          // ws fields
          ...ws,
  ...
  public async delete(token: IToken, parentWsIds: IKVObject, id: number): Promise<number> {
    try {
      ...
      const result = await models.Doctors.destroy({
        where: { ...parentWsIds, id }
      });
...
export const doctorsDAO = new DoctorsDAO();
