...
const log = require('bows')('API:DAO', 'Drugstores');

export class DrugstoresDAO extends BaseDAO {
  constructor() {
    // no op
    super();
  }
  ...
  public async list(token: IToken, parentWsIds: IKVObject, params: IDrugstoresParams): Promise<IApiPagedResponse<IDrugstoresWS>> {
    try {
      ...
      const results = await models.Drugstores.findAndCountAll({
  ...
  public async get(token: IToken, parentWsIds: IKVObject, params: IDrugstoresParams): Promise<IDrugstoresWS> {
    try {
      ...
      const result = await models.Drugstores.findOne({
        where: searchObj.where,
        // GQL requires Sub-Object IDs
        include: this.getJoinIncludeAry(searchObj)
      });
  ...
  public async create(token: IToken, parentWsIds: IKVObject, ws: IDrugstoresWS): Promise<IDrugstoresWS> {
    try {
      ...
      const result = await models.Drugstores.create({
        // ws fields
        ...ws,
        // parent resource IDs
        ...parentWsIds,
  ...
  public async update(token: IToken, parentWsIds: IKVObject, id: number, ws: IDrugstoresWS): Promise<number> {
    try {
      ...
      const result = await models.Drugstores.update(
        {
          // ws fields
          ...ws,
  ...
  public async delete(token: IToken, parentWsIds: IKVObject, id: number): Promise<number> {
    try {
      ...
      const result = await models.Drugstores.destroy({
        where: { ...parentWsIds, id }
      });
...
export const drugstoresDAO = new DrugstoresDAO();
